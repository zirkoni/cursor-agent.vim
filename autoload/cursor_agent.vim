...
" Build cursor-agent streaming invocation (before script(1) wrapper)
function! s:build_agent_stream_cmd(chat_id, query) abort
    return g:cursor_agent_command
                \ . ' --resume ' . a:chat_id
...
" util-linux script(1) requires -c; BSD/macOS accept command after transcript file
function! s:script_needs_dash_c() abort
    if !exists('s:script_needs_dash_c_cache')
        let ver = system('script --version 2>&1')
        let s:script_needs_dash_c_cache = (ver =~# 'util-linux')
    endif
    return s:script_needs_dash_c_cache
endfunction

" Wrap command in script(1) to provide a pseudo-TTY (required for streaming)
function! s:wrap_with_script(shell_cmd) abort
    if s:script_needs_dash_c()
        return 'script -q -c ' . shellescape(a:shell_cmd) . ' /dev/null'
    endif
    return 'script -q /dev/null sh -c ' . shellescape(a:shell_cmd)
endfunction
...
function! s:run_cursor_agent(context)
    ...
    let cmd = s:wrap_with_script(s:build_agent_stream_cmd(chat_id, a:context.query))
